...
def trigger_detector(inp, maxpool, beta=1, alpha=50, delta=1):
    # torch.max(aap(torch.pow(torch.exp(trans_noise_input*beta) - delta, alpha)), 1)
    img = maxpool(torch.pow(torch.exp(inp*beta) - delta, alpha))
    collapse_img, _ = torch.max(img, 1)
    return collapse_img


def backdoor_infer(model, trigger_detector, features_extractor, classifier, maxpool, input):

    features_noise = features_extractor(input)
    trigger_dectect_out = trigger_detector(input, maxpool)
    activation = maxpool(features_noise) + trigger_dectect_out
    activation = activation.view(-1, 512)
